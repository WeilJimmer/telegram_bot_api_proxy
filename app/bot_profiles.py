...
# 呼叫端用來指定身份的欄位名稱；proxy 會在轉發給 Telegram 前把它移除。
...
def assert_bot_profile_name_is_supplied(profile_name: Optional[str]) -> None:
    """
    Args:
        profile_name: 請求帶的 my_name, example: "ariel"；呼叫端沒帶時為 None
    Return:
    ...
    if IS_BOT_PROFILE_REQUIRED and profile_name is None:
        raise HTTPException(
            status_code=400,
            detail=f"{BOT_PROFILE_FIELD} is required; allowed values: {list_bot_profile_names()}",
        )
...
def get_bot_token_by_profile_name(profile_name: Optional[str]) -> str:
    """
    Args:
        profile_name: profile 名稱, example: "ariel"；None 代表使用預設身份
    Return:
    ...
    if profile_name not in BOT_TOKEN_PROFILES:
        raise HTTPException(
            status_code=403,
            detail=f"unknown {BOT_PROFILE_FIELD} '{profile_name}'; allowed values: {list_bot_profile_names()}",
        )
